package testCases;

import GenericUtility.BaseClass;
import org.testng.Assert;
import org.testng.annotations.Test;
import pageObjects.*;

public class tc008ProductComaparisionLink  extends BaseClass {


    @Test
    void verify_linkInSuceesMessage() {
        HomePage hm = new HomePage(driver);
        hm.getMyAccount();
        hm.getLogin();

        LoginPage lg = new LoginPage(driver);

        lg.setTxtEmail(p.getProperty("email"));
        lg.setTxtPassword(p.getProperty("password"));
        lg.setBtnLogin();

        MyAccountPage mp = new MyAccountPage(driver);

        mp.setSearchBar(p.getProperty("searchProductName"));
        mp.setSearchbtn();

        IPhonePage ip = new IPhonePage(driver);
        ip.setAddToCompare();
        ip.setLink();

        ProductComparisionPage pg = new ProductComparisionPage(driver);
        String message = pg.setProductComparison();
        Assert.assertEquals(message, "Product Comparison");
    }
}
